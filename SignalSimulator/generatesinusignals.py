...
information_path = os.path.join(script_dir, "sinusmall.csv") #saves all the information for the files

f1 = open(information_path, "wb")
planets = csv.writer(f1)
planets.writerow(["SNR", "Noise_frac_in", "Period_in",  "Period_out", "A","A1", "A2", "A3", "A4", "A5", "A6"])
...
np.random.seed(300)
fact= np.random.lognormal(mean=0.15, sigma=0.08, size=numsim)
...
alldt0frac = np.random.rand(numsim)*(dt0fracmax-dt0fracmin)+dt0fracmin
...
for i in range(0, numsim):
	...
	entries = np.append(np.array([snr, fact[i],  period, simple_period]), amplitudes)
